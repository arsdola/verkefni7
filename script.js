...
function start() {
  play();
  x = confirm("Viltu spila annan leik?");
  if (x == false){
    alert(getResults());
  }
  while (x == true) {
...
function play() {
  ...
  x = prompt("Veldu tölu á milli 0-100");
  if (x == null){
    alert(getResults());
  }
  y = parseGuess(x);
  alert(getResponse(y,random));
  var agiskun = 1;
  while (getResponse(y, random) != 'Rétt') {
    x = prompt("Veldu aðra tölu frá 0-100");
    if (x == null) {
      alert("Hætt í leik");
      agiskun = 0;
      alert(getResults());
...
function getResponse(guess, correct){
  ...
  var talan = Math.abs(correct - guess);
  if (talan < 5) {
    return 'Mjög nálægt';
  } else if (talan < 10) {
    return 'Nálægt';
